...
int main(int argc, char **argv) {
	...
	RedisConnectionTL::initialize(argv[1], atoi(argv[2]));
	if (!RedisConnectionTL::instance().isConnected()) {
		cout << "failed to connect." << endl;
		return -1;
	} else {
		InstancesUtil::initialize(argv[1], atoi(argv[2]));
		myNodeId = InstancesUtil::instance().getNewInstanceId("Test-App");
		cout << "my instance ID: " << myNodeId << endl;
		if (!BroadcastUtil::initializeWithId("Test-App", myNodeId)) {
			cout << "failed to initialize broadcast framework" << endl;
			return -1;
		}
		if (InstancesUtil::instance().registerInstanceUpCallback("Test-App", myNewInstanceCallback) == -1) {
			BroadcastUtil::stopAll();
			cout << "failed to register callback for new instance notifications" << endl;
			return -1;
		}
		if (InstancesUtil::instance().registerInstanceDownCallback("Test-App", myInstanceDownCallback) == -1) {
			BroadcastUtil::stopAll();
			cout << "failed to register callback for instance down notifications" << endl;
			return -1;
		}
		if (InstancesUtil::instance().publishNodeDetails("Test-App",myNodeId, "localhost", 2039, 60) == -1) {
			BroadcastUtil::stopAll();
	...
	while (!done) {
		...
		if (isSubscribed) {
			...
			if (strstr(message, "subscribe") == message || strstr(message, "SUBSCRIBE") == message) {
				if (BroadcastUtil::instance().addSubscription(message+strlen("subscribe "), myCallback) != 1) {
				...
				if (BroadcastUtil::instance().removeSubscription(message+strlen("unsubscribe ")) != 1) {
				...
				BroadcastUtil::instance().publish(command.substr(channel+1, payload-channel-1).c_str(), command.substr(payload+1).c_str());
				continue;
				...
				int res = InstancesUtil::instance().getFastLock("Test-App", "test-lock", 60);
				if (res == 0) {
				...
				int res = InstancesUtil::instance().releaseFastLock("Test-App", "test-lock");
				if (res == 0) {
		...
		RedisResult res = RedisConnectionTL::instance().cmd(message);
		if (res.resultType() == FAILED || !RedisConnectionTL::instance().isConnected()) {
			done = true;
		} else {
			printResult(res);
		}
	}
	if (InstancesUtil::instance().removeNodeDetails("Test-App",myNodeId) == 0) {
